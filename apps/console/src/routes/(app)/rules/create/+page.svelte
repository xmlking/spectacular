...
<Form {superform} submitButtonText="Create" class="space-y-6">
	<div class="mb-6 grid gap-6 md:grid-cols-3 lg:grid-cols-6">
		...
		<div class="col-span-3">
			<Select
				bind:justValue={$form.source}
				name="source"
				itemId="displayName"
				label="displayName"
				placeholder="Select Source or Local"
				{loadOptions}
				--list-z-index="100"
				--border-radius="8px"
			...
			/>
			<Helper class="mt-2 text-sm italic">
				Format: Prefix entry with "device:" or "pool:" to filter by device or pool OR enter
				an IP Address
			</Helper>
			{#if $errors['rule.source']}
				<Helper class="mt-2 text-sm italic text-red-500">
					{$errors['rule.source']}
				</Helper>
			{/if}
		</div>
		<div class="col-span-3">
			<FloatingTextInput field={keys.sourcePort} label="Source or Local Port" />
		</div>
		<div class="col-span-3">
			<Select
				bind:justValue={$form.destination}
				name="destination"
				itemId="displayName"
				label="displayName"
				{loadOptions}
				placeholder="Select Destination or Remote"
				--list-z-index="100"
				--border-radius="8px"
				--border-focused="1px solid blue"
			/>
		...
		<div class="col-span-3">
			<FloatingTextInput field={keys.destinationPort} label="Destination or Remote Port" />
		</div>
		<div>
