...
<div bind:this={elemDocSearch} class="modal-search {cBase}">
  ...
  <header class="modal-search-header {cHeader}">
    <Search size={24} strokeWidth={2.7} class="ml-4 text-xl" />
    <input
      class={cSearchInput}
  ...
  {#if results.length > 0}
    <nav class="list-nav {cResults}" tabindex="-1">
      {#each results as category}
        <div class="p-4 text-sm font-bold">{category.title}</div>
        <ul>
          {#each category.list as link}
            <li class="text-lg">
              ...
              >
                ...
                <span class="hidden text-xs opacity-50 md:block">{link.href}</span>
              </a>
            </li>
          {/each}
        </ul>
