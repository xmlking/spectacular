...
const { posted, allErrors, form, errors, enhance, delayed, message, reset, tainted, submitting } = superform;
  ...
  {#if showButtons}
    <ButtonGroup>
      <Button outline on:click={() => history.back()}>
        <ArrowLeftOutline width="18" height="18" class="mr-2 text-blue-500 dark:text-green-500" />{backButtonText}
      </Button>
