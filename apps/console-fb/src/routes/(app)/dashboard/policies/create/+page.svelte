...
<Form {superform} submitButtonText="Create" class="space-y-6">
  ...
  <div class="mb-6 grid gap-6 md:grid-cols-3 lg:grid-cols-6">
    ...
    <div class="col-start-5 flex justify-end">
      <Toggle field="rule.shared" class="toggle-secondary toggle" labelPosition="before" {disabled}>Shared</Toggle>
    </div>
    <div class="col-end-7">
    ...
    <div class="col-span-2">
      <Range field="rule.throttleRate" class="range-primary range-md" min="0" max="100" label="Bandwidth limit"
        >Throttle Rate {$form.rule.throttleRate}</Range
      >
    </div>
    <div class="flex justify-start">
      <Toggle field={keys.active} class="toggle-secondary toggle" labelPosition="before">Active</Toggle>
    </div>
