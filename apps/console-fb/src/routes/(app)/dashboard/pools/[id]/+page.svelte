...
const inPoolColumns = inPoolTable.createColumns([
  ...
  inPoolTable.column({
    ...
    cell: ({ value }) => createRender(RemoveDevicePoolButton, { id: value }).on('delete', handleDelete),
    plugins: {
      tableFilter: {
        exclude: true,
      },
...
const notInPoolColumns = notInPoolTable.createColumns([
  ...
  notInPoolTable.column({
    ...
    cell: ({ value }) => createRender(AddDevicePoolButton, { poolId, deviceId: value }).on('add', handleAdd),
    plugins: {
      tableFilter: {
        exclude: true,
      },
...
{#if inPool && notInPool}
  <Navbar class="rounded border">
    <NavBrand>
      <MobilePhoneOutline />
      <span class="ml-2 self-center whitespace-nowrap text-xl font-semibold dark:text-white"> Devices in Pool </span>
    </NavBrand>
  </Navbar>
  ...
  <Navbar class="rounded border">
    <NavBrand>
      <MobilePhoneOutline />
      <span class="self-center whitespace-nowrap text-xl font-semibold dark:text-white"> Devices NOT in Pool </span>
    </NavBrand>
  </Navbar>
