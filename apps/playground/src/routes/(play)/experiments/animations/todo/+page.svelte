...
<div>
  {#each todos as todo}
    ...
    >
      ...
      <button class="variant-filled-error btn" type="button" on:click={() => removeTodo(todo.id)}>Remove</button>
    </div>
  {/each}
  ...
  <div class="flex flex-row gap-4 pt-4">
    <button type="button" on:click={() => addTodo(nTodo)} class="variant-filled-secondary btn">Add Todo</button>
    <a href="/experiments/animations" class="btn bg-secondary-100 text-surface-900">Home</a>
    <a href="https://svelte.dev/docs/svelte-animate" target="_blank" class="btn bg-secondary-100 text-surface-900"
      >Learn More</a
    >
  </div>
</div>
